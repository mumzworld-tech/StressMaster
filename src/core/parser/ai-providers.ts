  ...
  implements SmartAIProvider
{
  ...
  async generateCompletion(
    ...
    return this.retryOperation(async () => {
      ...
      console.log("  Endpoint:", `${this.config.endpoint}/chat/completions`);
      console.log("  Model:", request.model || this.config.model);
...
export class AIProviderFactory {
  ...
  static create(config: AIConfig): AIProvider {
    ...
    switch (config.provider) {
      ...
      default:
        throw new Error(
          `Unsupported AI provider: ${config.provider}. Supported providers: ollama, openai, claude, gemini, openrouter`
        );
    }
  }
  ...
  static getSupportedProviders(): AIProviderType[] {
    return ["ollama", "openai", "claude", "gemini", "openrouter"];
  }
